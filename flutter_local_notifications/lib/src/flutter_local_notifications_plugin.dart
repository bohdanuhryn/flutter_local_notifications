...
class FlutterLocalNotificationsPlugin {
  ...
  Future<void> schedule(int id, NotificationContent content,
    ...
    if (_platform.isAndroid) {
      await resolvePlatformSpecificImplementation<
              AndroidFlutterLocalNotificationsPlugin>()
          .schedule(
              id, content, scheduledDate, notificationDetails?.android,
              payload: payload, androidAllowWhileIdle: androidAllowWhileIdle);
    } else if (_platform.isIOS) {
      await resolvePlatformSpecificImplementation<
              IOSFlutterLocalNotificationsPlugin>()
          ?.schedule(id, content, scheduledDate, notificationDetails?.iOS,
              payload: payload);
    }
  }
  ...
  Future<void> showEveryFewDaysAtTime(int id, NotificationContent content,
      NotificationDetails notificationDetails, Time time,
      {String payload, int days = 1}) async {
    if (_platform.isAndroid) {
  ...
  Future<void> showDailyAtTime(int id, NotificationContent content,
    ...
    if (_platform.isAndroid) {
      await resolvePlatformSpecificImplementation<
              AndroidFlutterLocalNotificationsPlugin>()
          ?.showDailyAtTime(
              id, content, notificationTime, notificationDetails?.android,
              payload: payload);
    } else if (_platform.isIOS) {
      await (FlutterLocalNotificationsPlatform.instance
              as IOSFlutterLocalNotificationsPlugin)
          ?.showDailyAtTime(
              id, content, notificationTime, notificationDetails?.iOS,
              payload: payload);
    }
  }
  ...
  Future<void> showWeeklyAtDayAndTime(int id, NotificationContent content,
    ...
    if (_platform.isAndroid) {
      await resolvePlatformSpecificImplementation<
              AndroidFlutterLocalNotificationsPlugin>()
          ?.showWeeklyAtDayAndTime(id, content, day, notificationTime,
      ...
      await resolvePlatformSpecificImplementation<
          ...
          ?.showWeeklyAtDayAndTime(
              id, content, day, notificationTime, notificationDetails?.iOS,
              payload: payload);
    }
  }
